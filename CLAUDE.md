...
## 0. 靈魂校準 (Soul Alignment)
- **必須優先閱讀並遵守中央憲法：** `./brain/identity.md`
- **調用 Auto-Skill 共享經驗：** `./brain/auto-skill/`
...
### Superpowers 工程紀律 (Engineering Discipline)
當執行開發、除錯或規劃任務時，必須強制調用 `./brain/superpowers/` 中的規範：

- **TDD 優先**：嚴禁先寫代碼再寫測試。若違反，必須刪除代碼重新開始。
- **兩階段審查**：先由 Subagent 審查「規格符合性」，再審查「代碼品質」。
- **拒絕表演性贊同**：禁止說「好主意！」或「妳說得對！」。收到意見後必須先「技術複述」與「驗證」，若意見錯誤必須主動推回。
- **驗證先行**：在宣稱「修好了」或「已完成」前，必須提供最新的驗證證據（證據優於感覺）。
